#!/bin/bash
# GATE 2 (UNBLINDING) — observed + postfit-expected AsymptoticLimits per
# point: ONE direct UNMASKED b-only fit (SR open, r frozen 0, all nine
# nuisances floating) -> observed limit from data + expected band from the
# snapshot. Interlock: last argument must be the literal GATE1-CLOSED.
# Derived from the blinded fullsyst_limit_v3 recipe; only the masks and the
# AsymptoticLimits run mode differ. Original header follows.
...
if [[ "$#" -ne 6 ]]; then
    echo "usage: $0 <mode> <width> <mass_GeV> <rMax> <fwd_tf> GATE1-CLOSED" >&2
    exit 2
fi
MODE="$1"
WIDTH="$2"
...
FWD_TF="$5"
INTERLOCK="$6"
case "${MODE}" in gate2_observed_limit) ;; *) echo "bad mode" >&2; exit 4 ;; esac
[[ "${INTERLOCK}" == "GATE1-CLOSED" ]] || {
    echo "GATE2_INTERLOCK: refusing observed limits — Gate 1 not confirmed closed" >&2
    exit 90
}
...
RESULT="${SCRATCH}/gate2_observed_result"
RESULT_ARCHIVE="${SCRATCH}/gate2_observed_result.tgz"
...
finalize_result() {
    ...
    {
        echo "campaign=gate2_observed_limit"
        echo "fwd_tf=${FWD_TF}"
        echo "exit_code=${rc}"
        ...
        echo "rMax=${RMAX}"
        echo "signal=${SIGNAL}"
        echo "fit=direct_UNMASKED_bonly_all_nine_nuisances_floating"
        echo "sr_channels_open=all_four_pass_region1"
        echo "limit_scope=observed_plus_postfit_expected"
...
(
    ...
    M_ALL_OFF="mask_cen_Cen24Pass_Region1=0,mask_cen_Cen25Pass_Region1=0,mask_fwd_Fwd24Pass_Region1=0,mask_fwd_Fwd25Pass_Region1=0"
    combine -M MultiDimFit -d workspace.root -m 0 \
        --rMin 0 --rMax "${RMAX}" \
        --setParameters "r=0,${M_ALL_OFF}" \
...
PY
    ...
    combine -M AsymptoticLimits \
        ...
        --snapshotName MultiDimFit --bypassFrequentistFit -m 0 \
    ...
    python3 "${SCRATCH}/validate_observed_limit.py" \
