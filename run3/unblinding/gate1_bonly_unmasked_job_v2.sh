#!/bin/bash
# GATE 1 v2 (UNBLINDING) — unmasked b-only fit on data. v2 = v1 with the
# 1.4 bound check demoted to record-and-warn per ruling A (see PLAN.md). r frozen at 0. No s+b
...
# v2 (ruling A, 2026-08-14, PLAN.md decision log): bound proximity is
# RECORDED as a documented deviation, not a job failure — the railing is a
# blinded-side condition (masked fit had both fwd par2 at exactly -50) that
# SR data relieve. The violation list stays in gate1_pulls.json for the record.
if bad:
    print(f"GATE 1.4 DEVIATION (ruling A): rpf params near bounds: {[b['name'] for b in bad]}")
else:
