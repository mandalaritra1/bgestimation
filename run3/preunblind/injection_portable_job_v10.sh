...
run_fit() {
    ...
    # First profile the nuisances with r fixed at that requested start.  The
    # accepted physical-range fit below then reloads only its matching snapshot
    # and floats r.  This avoids borrowing a preferred nuisance optimum from a
    # different seed while keeping the final signal-strength fit unbiased.
