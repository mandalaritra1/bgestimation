#!/bin/bash
# Isolated numerical diagnostics for three prescribed shape-nuisance freeze
...
case "${MODE}" in
    all_shapes_frozen)
        FROZEN_SHAPES="jes,jer,pileup,pdf,q2,ttag_pt1"
        FROZEN_SHAPE_VALUES="jes=0,jer=0,pileup=0,pdf=0,q2=0,ttag_pt1=0"
        FLOATING_SHAPES="none"
        ;;
    theory_only_floating)
        FROZEN_SHAPES="jes,jer,pileup,ttag_pt1"
        FROZEN_SHAPE_VALUES="jes=0,jer=0,pileup=0,ttag_pt1=0"
        FLOATING_SHAPES="pdf,q2"
        ;;
    experimental_only_floating)
        FROZEN_SHAPES="pdf,q2"
        FROZEN_SHAPE_VALUES="pdf=0,q2=0"
        FLOATING_SHAPES="jes,jer,pileup,ttag_pt1"
        ;;
    *)
...
CAMPAIGN="${SCRATCH}/shape_freeze_modes_v1_${MODE}_campaign"
RESULT="${SCRATCH}/shape_freeze_modes_v1_result"
RESULT_ARCHIVE="${SCRATCH}/shape_freeze_modes_v1_result.tgz"
...
finalize_result() {
    ...
    {
        echo "diagnostic=shape_freeze_modes_v1"
        echo "mode=${MODE}"
    ...
    echo "SHAPE_FREEZE_MODES_V1_DONE mode=${MODE} rc=${wrapper_rc} result=$(basename "${RESULT_ARCHIVE}")"
    exit "${wrapper_rc}"
}
...
for expected in \
    ...
    grep -Fxq "${expected}" "${PAYLOAD_WORK}/payload_provenance.txt" || {
        echo "SHAPE_FREEZE_MODES_V1_INVALID payload_provenance=${expected}" >&2
        exit 8
    }
done
(
...
print("SHAPE_FREEZE_MODES_V1_RUNTIME_OK", ROOT.gROOT.GetVersion(), PhysicsModel.__name__)
PY
...
for required in \
    ...
    if [[ ! -s "${required}" ]]; then
        echo "SHAPE_FREEZE_MODES_V1_INVALID missing_or_empty=${required}" >&2
        exit 10
    fi
done
if [[ -e "${AREA}" ]]; then
    echo "SHAPE_FREEZE_MODES_V1_REFUSE_EXISTING area=${AREA}" >&2
    exit 11
fi
...
echo "SHAPE_FREEZE_MODES_V1_BOOTSTRAP_OK mode=${MODE} width=${WIDTH} mass=${MASS}"
