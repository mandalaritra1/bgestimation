...
# v2: the fwd TF order became an F-test-selected parameter (2026-08-14 verdict:
# 2x0). --fwd-tf overrides the fwd entry so harvested area names match the
# order actually built into the archive's workspace.
TF_ORDERS = {"cen2425": "2x2", "fwd2425": "2x1"}
...
class Point:
    ...
    def tf_order(self) -> str:
        return TF_ORDERS[self.category]
...
def validate_archive(source: Path, point: Point) -> dict[str, object]:
    ...
    try:
        with tarfile.open(source, "r:gz") as archive:
            ...
            # v2: archives from workspace_portable_job_v2.sh declare the TF
            # order they were built with; when present it must match the order
            # this harvest names the area with.
            if "tf_order" in metadata and metadata["tf_order"] != point.tf_order:
                raise ValueError(
                    f"{source}: workspace.ok tf_order={metadata['tf_order']!r}, "
                    f"expected {point.tf_order!r}"
                )
...
def main() -> int:
    ...
    parser.add_argument("--fwd-tf", default="2x1",
                        help="TF order of the fwd2425 areas being harvested (default 2x1)")
    args = parser.parse_args()
    if args.fwd_tf not in {"0x0", "0x1", "0x2", "1x0", "1x1", "1x2", "2x0", "2x1", "2x2", "3x0", "3x1"}:
        parser.error(f"unsupported --fwd-tf: {args.fwd_tf}")
    TF_ORDERS["fwd2425"] = args.fwd_tf
