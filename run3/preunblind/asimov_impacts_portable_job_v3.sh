#!/bin/bash
# Portable one-nuisance synthetic-Asimov Impacts worker.  The EXIT trap always
...
case "${NUISANCE}" in lumi24|lumi25|ttbar_xsec) ;; *) echo "unsupported nuisance: ${NUISANCE}" >&2; exit 2 ;; esac
IMPACTS_NAME="asimov_w${WIDTH}_m${MASS}_${NUISANCE}"
TOY_SEED=123456
SCRATCH="${_CONDOR_SCRATCH_DIR:?missing _CONDOR_SCRATCH_DIR}"
PAYLOAD="${SCRATCH}/asimov_impacts_payload_w${WIDTH}_m${MASS}_v2.tgz"
...
for expected in \
    ...
    "model_nuisances=lumi24,lumi25,ttbar_xsec" "dataset_scope=synthetic_asimov_only" \
...
COMMON=( -M Impacts -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0
    ...
    --robustFit 1 --saveFitResult --cminDefaultMinimizerStrategy 1
