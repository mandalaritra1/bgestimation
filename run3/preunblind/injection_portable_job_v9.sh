...
FIT_START_MODE="independent_wide_bootstrap_then_physical_multidimfit"
...
finalize_result() {
    ...
    fi
    {
        ...
        echo "bootstrap_seed_policy=independent_per_start"
        echo "final_nuisance_start=matching_bootstrap_snapshot_no_override"
...
run_fit() {
    local start_label="$1"
    local start_value="$2"
    local bootstrap_fit="multidimfit_bootstrap_${start_label}.root"
    local bootstrap_snapshot="higgsCombine_bootstrap_${start_label}.MultiDimFit.mH0.root"
    ...
    # The POI-only stage moves r before the RPF coefficients are released,
    # avoiding invalid negative-yield steps without borrowing a preferred
    # nuisance optimum from a different signal-strength seed.
    CURRENT_STAGE="bootstrap_${start_label}"
    combine -M MultiDimFit -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0 \
        ...
        --cminFallbackAlgo Minuit2,Simplex,0:0.1 --robustHesse 1 \
        --saveNLL --saveWorkspace --saveFitResult \
        -n "_bootstrap_${start_label}" \
        > "bootstrap_${start_label}.log" 2>&1

    CURRENT_STAGE="validate_bootstrap_${start_label}"
    python3 "${REPO}/run3/validate_fit_result.py" \
        "${bootstrap_fit}" --key fit_mdf --min-cov-qual 3 \
        > "bootstrap_${start_label}_validation.log" 2>&1

    CURRENT_STAGE="fit_${start_label}"
    combine -M MultiDimFit \
        -d "${bootstrap_snapshot}" \
...
for required in \
    ...
    bootstrap_zero_validation.log bootstrap_injected_validation.log bootstrap_halfmax_validation.log \
    fit_zero_validation.log fit_injected_validation.log fit_halfmax_validation.log \
    fit_zero_range_validation.log fit_injected_range_validation.log fit_halfmax_range_validation.log; do
