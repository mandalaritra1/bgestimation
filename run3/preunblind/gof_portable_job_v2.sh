#!/bin/bash
# Portable masked saturated-GoF worker (v2: parameterized matrix points on
# the FULL-SYSTEMATICS fullsyst_matrix_20260813 snapshots + MaxCalls).  The EXIT trap always returns a
...
PAYLOAD="${SCRATCH}/gof_payload_w${WIDTH}_m${MASS}.tgz"
...
combine -M GoodnessOfFit -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0 \
    --algo saturated --rMin 0 --rMax "${RMAX}" --X-rtd MINIMIZER_MaxCalls=5000000 \
