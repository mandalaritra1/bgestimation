#!/bin/bash
# Portable one-nuisance synthetic-Asimov Impacts canary.  The EXIT trap always
...
NUISANCE="lumi24"
IMPACTS_NAME="asimov_lumi24"
TOY_SEED=123456
SCRATCH="${_CONDOR_SCRATCH_DIR:?missing _CONDOR_SCRATCH_DIR}"
PAYLOAD="${SCRATCH}/asimov_impacts_payload_w1_m2000.tgz"
...
for expected in \
    "width=${WIDTH}" "mass_GeV=${MASS}" "production_rMax=${RMAX}" "rInject=${RINJECT}" \
    "named_nuisance=${NUISANCE}" "dataset_scope=synthetic_asimov_only" \
    "pass_region_masks=all_off_frozen"; do
...
INITIAL_SEEDED="higgsCombine_initialFit_${IMPACTS_NAME}.MultiDimFit.mH0.${TOY_SEED}.root"
...
PARAM_SEEDED="higgsCombine_paramFit_${IMPACTS_NAME}_${NUISANCE}.MultiDimFit.mH0.${TOY_SEED}.root"
PARAM_ROOT="higgsCombine_paramFit_${IMPACTS_NAME}_${NUISANCE}.MultiDimFit.mH0.root"

normalize_seeded_result() {
    local seeded="$1"
    local normalized="$2"
    [[ -s "${seeded}" && ! -e "${normalized}" ]] || {
        echo "unexpected seeded-result state: ${seeded} -> ${normalized}" >&2; return 1;
    }
    mv "${seeded}" "${normalized}"
}
...
normalize_seeded_result "${INITIAL_SEEDED}" "${INITIAL_ROOT}"
...
normalize_seeded_result "${PARAM_SEEDED}" "${PARAM_ROOT}"
...
combineTool.py "${COMMON[@]}" -o impacts_lumi24.json > collect_impacts.log 2>&1
CURRENT_STAGE="validate_impacts_json"
python3 "${VALIDATOR}" impacts impacts_lumi24.json --nuisance "${NUISANCE}" \
