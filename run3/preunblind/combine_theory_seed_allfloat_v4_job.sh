...
CAMPAIGN="${SCRATCH}/theory_seed_allfloat_v4_${MODE}_campaign"
RESULT="${SCRATCH}/theory_seed_allfloat_v4_result"
RESULT_ARCHIVE="${SCRATCH}/theory_seed_allfloat_v4_result.tgz"
...
finalize_result() {
    ...
    {
        echo "diagnostic=theory_seed_allfloat_v4"
        echo "mode=${MODE}"
    ...
    echo "THEORY_SEED_ALLFLOAT_V4_DONE mode=${MODE} rc=${wrapper_rc} result=$(basename "${RESULT_ARCHIVE}")"
    exit "${wrapper_rc}"
}
...
for expected in \
    ...
    grep -Fxq "${expected}" "${PAYLOAD_WORK}/payload_provenance.txt" || {
        echo "THEORY_SEED_ALLFLOAT_V4_INVALID payload_provenance=${expected}" >&2
        exit 8
    }
done
(
...
print("THEORY_SEED_ALLFLOAT_V4_RUNTIME_OK", ROOT.gROOT.GetVersion(), PhysicsModel.__name__)
PY
...
for required in \
    ...
    if [[ ! -s "${required}" ]]; then
        echo "THEORY_SEED_ALLFLOAT_V4_INVALID missing_or_empty=${required}" >&2
        exit 10
    fi
done
if [[ -e "${AREA}" ]]; then
    echo "THEORY_SEED_ALLFLOAT_V4_REFUSE_EXISTING area=${AREA}" >&2
    exit 11
fi
...
echo "THEORY_SEED_ALLFLOAT_V4_OK mode=${MODE} width=${WIDTH} mass=${MASS}"
