#!/usr/bin/env python3
"""Static contract test for the two prescribed v2 shape-freeze diagnostics."""
...
WORKER = HERE / "combine_bootstrap_shape_freeze_modes_v2_job.sh"
MATRIX = HERE / "combine_bootstrap_shape_freeze_modes_v2.tsv"
SUBMIT = HERE / "combine_bootstrap_shape_freeze_modes_v2.sub"

EXPECTED = {
    "theory_plus_jesjer": {
        "frozen": "pileup,ttag_pt1",
        "values": "pileup=0,ttag_pt1=0",
        "floating": "pdf,q2,jes,jer",
    },
    "theory_plus_pileup_ttag": {
        "frozen": "jes,jer",
        "values": "jes=0,jer=0",
        "floating": "pdf,q2,pileup,ttag_pt1",
    },
}
...
def main() -> int:
    ...
    assert points == [
        ("theory_plus_jesjer", "1", "2000", "1"),
        ("theory_plus_pileup_ttag", "1", "2000", "1"),
    ]
    ...
    assert "diagnostic=shape_freeze_modes_v2" in worker
    ...
    assert (
        "arguments = combine_bootstrap_shape_freeze_modes_v2_job.sh "
    ...
    assert "combine_bootstrap_shape_freeze_modes_v2.tsv" in submit

    namespace = "fullsyst_canary_v1/diagnostics/shape_freeze_modes_v2"
    ...
    assert remap_lines == [
        'transfer_output_remaps = "shape_freeze_modes_v2_result.tgz = '
        f'{remap_template}"'
    ]
    remaps = {
    ...
    assert remaps == {
        f"{namespace}/theory_plus_jesjer/w1_m2000.tgz",
        f"{namespace}/theory_plus_pileup_ttag/w1_m2000.tgz",
    }
    assert len(remaps) == 2
    log_lines = [
    ...
    print("COMBINE_BOOTSTRAP_SHAPE_FREEZE_MODES_V2_CONTRACT_TEST_OK")
    return 0
