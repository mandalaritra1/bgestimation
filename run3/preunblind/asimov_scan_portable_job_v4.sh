#!/bin/bash
# Portable synthetic-Asimov 1D likelihood-scan (v4: parameterized points on
# the fullsyst_matrix_20260813 snapshots, strategy 0 + MaxCalls).  The EXIT trap always
# returns a diagnostic archive, including partial artifacts and their hashes.
set -euo pipefail

if [[ "$#" -ne 5 ]]; then
    echo "usage: $0 <width> <mass> <production_rmax> <rinject> <scan_rmax>" >&2
    exit 2
fi
...
RINJECT="$4"
SCAN_RMAX="$5"
case "${WIDTH}" in 1|10|30) ;; *) echo "bad width" >&2; exit 2 ;; esac
case "${MASS}" in 2000|4000|6000|7000) ;; *) echo "bad mass" >&2; exit 2 ;; esac
GRID_POINTS=41
TOY_SEED=123456
...
VALIDATOR="${SCRATCH}/validate_portable_asimov_scan.py"
...
for expected in \
    "width=${WIDTH}" "mass_GeV=${MASS}" "rMax=${PRODUCTION_RMAX}" \
    ; do
...
combine -M MultiDimFit -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0 \
    ...
    --setParameters "r=${RINJECT},${M_OFF}" --freezeParameters "${M_FRZ}" \
    --cminDefaultMinimizerStrategy 0 --cminPreScan --cminPreFit 1 --X-rtd MINIMIZER_MaxCalls=5000000 \
