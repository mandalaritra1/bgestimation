#!/bin/bash
# Portable Asimov injection worker (v17 = v16 with the strategy-0 primary
# minimizer and per-fit EDM acceptance 0.1 (Minuit's own convergence
# criterion): at sparse high-mass points, boundary-pinned bin parameters
# put a ~0.07 numerical-noise floor under the EDM estimate while the
# minimum itself is stable to 1e-7 (see state/triage_v16_w1m6000).
# Binding physics gates are cross-start r-hat/NLL agreement at harvest.
# (v16 = v15 + robustFit profile errors on r,
# the boundary-aware injection gate (status 0/1, covQual>=2, EDM<=0.01;
# validate_injection_fit.py), and no fail-fast: all three starts always run
# and validation failures are collected at the end), run on
...
run_fit() {
    ...
    if ! python3 "${SCRATCH}/validate_injection_fit.py" \
        "${fit_root}" --key fit_mdf --max-edm 0.1 \
        > "fit_${start_label}_validation.log" 2>&1; then
        VALIDATION_FAILURES="${VALIDATION_FAILURES} ${start_label}"
    fi
...
if [[ -n "${VALIDATION_FAILURES}" ]]; then
    echo "INJECTION_JOB_INVALID failed_start_validations:${VALIDATION_FAILURES}" >&2
    exit 16
fi
for required in \
