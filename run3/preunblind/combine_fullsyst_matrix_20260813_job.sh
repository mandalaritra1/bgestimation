#!/bin/bash
# Full-systematics blinded expected-limit production job (12-point matrix).
# Stage 1 seeds with experimental shapes fixed and theory shapes floating;
# stage 2 restores all six shape nuisances in the standard masked
# physical-range refit; stage 3 computes the blinded expected limit from the
# stage-2 snapshot with the Pass windows unmasked (Asimov only, --run blind).
# All combine calls carry --X-rtd MINIMIZER_MaxCalls=5000000: the default
# 500*npar call budget is exceeded by these ~580-parameter fits and was the
# sole cause of the 2026-08-11/12 stage-2 "failures" (status 4 = call limit).
set -euo pipefail
...
case "${WIDTH}" in
    1|10|30) ;;
    *) echo "unsupported matrix width: ${WIDTH}" >&2; exit 3 ;;
esac
case "${MASS}" in
    2000|4000|6000|7000) ;;
    *) echo "unsupported matrix mass: ${MASS}" >&2; exit 3 ;;
esac
...
case "${MODE}" in
    ...
    *)
        echo "unsupported matrix mode: ${MODE}" >&2
        exit 4
        ;;
esac
...
CAMPAIGN="${SCRATCH}/fullsyst_matrix_20260813_${MODE}_campaign"
RESULT="${SCRATCH}/fullsyst_matrix_20260813_result"
RESULT_ARCHIVE="${SCRATCH}/fullsyst_matrix_20260813_result.tgz"
...
finalize_result() {
    ...
    if [[ -n "${AREA}" && -d "${AREA}" ]]; then
        for source_path in \
            ...
            "${AREA}/stage3_limit.log" \
            "${AREA}/stage3_limit_validation.log" \
            "${AREA}"/*stage1*.root \
            "${AREA}"/*stage2*.root \
            "${AREA}"/*stage3*.root; do
    ...
    {
        echo "campaign=fullsyst_matrix_20260813"
        echo "mode=${MODE}"
        ...
        echo "poi_fixed=r=0"
        echo "pass_region1_masks=on_and_frozen_stages_1_2_off_and_frozen_stage3"
        echo "rpf_bootstrap_range=-50,50"
        echo "minimizer_maxcalls=5000000"
        echo "stage3_method=AsymptoticLimits_blind_bypassFrequentistFit_snapshot"
        echo "stage3_accuracy=rRelAcc0.0005_rAbsAcc1e-9_strategy0"
        if [[ -s "${PAYLOAD}" ]]; then
    ...
    echo "FULLSYST_MATRIX_20260813_DONE mode=${MODE} rc=${wrapper_rc} result=$(basename "${RESULT_ARCHIVE}")"
    exit "${wrapper_rc}"
}
...
for expected in \
    ...
    grep -Fxq "${expected}" "${PAYLOAD_WORK}/payload_provenance.txt" || {
        echo "FULLSYST_MATRIX_20260813_INVALID payload_provenance=${expected}" >&2
        exit 8
    }
done
(
...
print("FULLSYST_MATRIX_20260813_RUNTIME_OK", ROOT.gROOT.GetVersion(), PhysicsModel.__name__)
PY
...
for required in \
    ...
    if [[ ! -s "${required}" ]]; then
        echo "FULLSYST_MATRIX_20260813_INVALID missing_or_empty=${required}" >&2
        exit 10
    fi
done
if [[ -e "${AREA}" ]]; then
    echo "FULLSYST_MATRIX_20260813_REFUSE_EXISTING area=${AREA}" >&2
    exit 11
fi
...
(
    ...
    combine -M MultiDimFit -d workspace.root -m 0 \
        ...
        --cminDefaultMinimizerStrategy 1 --cminPreScan --cminPreFit 1 --X-rtd MINIMIZER_MaxCalls=5000000 \
    ...
    combine -M MultiDimFit \
        ...
        --cminPreScan --cminPreFit 1 --cminDefaultMinimizerStrategy 2 --X-rtd MINIMIZER_MaxCalls=5000000 \
...
PY
    ...
    combine -M AsymptoticLimits \
        -d higgsCombine_stage2.MultiDimFit.mH0.root \
...
if find "${AREA}" -type f -name 'higgsCombine*.AsymptoticLimits*.root' ! -name 'higgsCombine_stage3.AsymptoticLimits.mH0.root' | grep -q .; then
    echo "FULLSYST_MATRIX_20260813_INVALID unexpected_limit_output=${AREA}" >&2
    exit 13
fi

echo "FULLSYST_MATRIX_20260813_OK mode=${MODE} width=${WIDTH} mass=${MASS}"
