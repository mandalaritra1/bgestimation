#!/bin/bash
# Portable one-nuisance synthetic-Asimov Impacts worker (v6: boundary-aware
# initial-fit gate (status 0/1, covQual>=2, EDM<=0.1 — s+b Asimov fits at
# the r boundary; same rationale as the injection gate) and the strategy-0
# solver used by the validated toy/limit recipes. (v4: nine-nuisance
...
COMMON=( -M Impacts -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0
    ...
    --robustFit 1 --saveFitResult --cminDefaultMinimizerStrategy 0
...
python3 "${SCRATCH}/validate_injection_fit.py" "${INITIAL_FIT}" --key fit_mdf --max-edm 0.1 \
