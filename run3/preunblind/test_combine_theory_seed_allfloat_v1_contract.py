...
WORKER = HERE / "combine_theory_seed_allfloat_v1_job.sh"
MATRIX = HERE / "combine_theory_seed_allfloat_v1.tsv"
SUBMIT = HERE / "combine_theory_seed_allfloat_v1.sub"
...
def main() -> int:
    ...
    assert matrix_bytes == b"theory_seed_allfloat_v1 1 2000 1\n"
    ...
    assert 'THEORY_SEED_ALLFLOAT_V1_REFUSE_EXISTING area=${AREA}' in worker

    assert (
        "arguments = combine_theory_seed_allfloat_v1_job.sh "
    ...
    assert "combine_theory_seed_allfloat_v1.tsv" in submit

    namespace = "fullsyst_canary_v2/diagnostics/theory_seed_allfloat_v1"
    remap_lines = [line for line in submit.splitlines() if line.startswith("transfer_output_remaps")]
    assert remap_lines == [
        'transfer_output_remaps = "theory_seed_allfloat_v1_result.tgz = '
    ...
    print("COMBINE_THEORY_SEED_ALLFLOAT_V1_CONTRACT_TEST_OK")
    return 0
