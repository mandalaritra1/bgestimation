...
MODEL_NUISANCES = ("jer", "jes", "lumi24", "lumi25", "pdf", "pileup", "q2", "ttag_pt1", "ttbar_xsec")  # alphabetical: compared against sorted()
...
def validate_snapshot(path: Path, rmax: float) -> dict[str, object]:
    ...
    try:
        ...
        # float32-stored bound
        if not math.isclose(float(poi.getMax()), rmax, rel_tol=1e-6):
