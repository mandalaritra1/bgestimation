...
run_fit() {
    local start_label="$1"
    local start_value="$2"
    local fit_name="injection_w${WIDTH}_m${MASS}_${INJECTION_LABEL}_${start_label}"
    ...
    # The standard Impacts initial-fit driver supplies robust profiling while
    # leaving r floating.  No start borrows a nuisance optimum from another,
    # and no RPF range is widened or otherwise changed.  Do not pass
    # the expectSignal option here: the toy file already defines the injected dataset,
    # and Combine otherwise overrides the requested initial value of r.
    ...
    combineTool.py -M Impacts -d "${SNAPSHOT}" --snapshotName MultiDimFit -m 0 \
        ...
        --freezeParameters "${M_FRZ}" \
        --redefineSignalPOIs r --named lumi24 \
        --robustFit 1 --saveFitResult -n "${fit_name}" --doInitialFit \
        > "fit_${start_label}.log" 2>&1
    [[ -s "${fit_root}" ]] || { echo "missing robust initial fit: ${fit_root}" >&2; exit 14; }
    [[ -s "${result_root}" ]] || { echo "missing robust initial result: ${result_root}" >&2; exit 15; }
